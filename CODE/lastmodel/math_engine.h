...
// ---- Substitution de 'x' par une valeur numérique ----
// Construit l'expression avec x remplacé et évalue
double evalAt(const char* expr, double xVal, bool& hasErr, char* errOut = nullptr) {
  char subExpr[EXPR_MAX * 3]; // buffer plus grand car on insère des nombres
  char xStr[12];
...
//  Retourne le résultat + remplit labelOut (ex: "sin", "arcsin")
// ============================================================
...
double applyFunc(char key, double val,
  ...
  switch (key) {
    ...
    // ---- LOG / 10^x ----
    case 'n': // mapped to 'l' key visually = log10
    ...
    // ---- LN / e^x ----
    case 'l': // mapped visually to 'ln' key
    ...
    // ---- PUISSANCE (clavier SCI) ----
    case '^':
      // géré directement dans l'expression; ne devrait pas arriver ici
      lbl = "^"; result = val;
      break;
